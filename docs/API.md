...
- `budget_min` / `budget_max` — фильтр по полю `typical_visit_cost_rub` у виноделен; при отсутствии оценки у точки она всё равно может попасть в пул.
- `start_lat` / `start_lon` — опционально; иначе используется ориентир (Краснодар) для погоды и старта маршрута.
...
Возвращает сохранённый маршрут текущего visitor (тот же cookie `turizm_vid`), что и при создании.

**Ответ 200** — формат как у `POST /api/routes/build/`: объекты `route` и `places`.

**Ошибки**
- `400` — некорректный UUID
- `404` — маршрут не найден для этого посетителя
...
- `503` — на сервере не задан `JWT_SECRET`
